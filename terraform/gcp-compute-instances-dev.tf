...
#/*
# make a static ip to use for this vm
resource "google_compute_address" "dev48hours" {
  name = "dev48hours"
}

# define the instance
resource "google_compute_instance" "dev48hours" {
  name                      = "dev48hours"
  machine_type              = "n1-standard-1"
  zone                      = var.gcp_zone
  tags                      = ["dev"]
  ...
  network_interface {
    network = google_compute_network.mynetwork.name
    access_config {
      nat_ip = google_compute_address.dev48hours.address
    }
  }
  service_account {
    scopes = ["userinfo-email", "compute-ro", "storage-ro", "cloud-platform"]
